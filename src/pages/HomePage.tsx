  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-6 max-w-md mx-auto">
      ...
      <div className="grid grid-cols-1 gap-4 w-full">
        {MODE_CARDS.map(({ to, icon: Icon, title, description, theme }) => (
          <Link
            key={to}
            to={to}
            className={cn(
              "flex items-center justify-center p-8 rounded-3xl transition-all shadow-sm border-2",
              theme.bg,
              theme.text,
              theme.border,
              theme.hoverBg,
